...
main() {
    ...
    cfuture<void> future2 = futures::then(future1, [](int x) {
        std::cout << x * 2 << '\n';
    });

    // Continuation to a std::future
    std::future<int> future3 = std::async([]() -> int { return 63; });
    cfuture<void> future4 = futures::then(future3, [](int x) {
        std::cout << x * 2 << '\n';
    });
    ...
    cfuture<int> future6 = then(future5, ex, [](int v) {
        return v * 2;
    });

    // Continuation with operator>>
    auto future7 = future6 >> [](int x) {
        return x * 2;
    };
