...
int main() {
    ...
    std::cout << f1c.get() << std::endl;

    // Unwrap tuple
    auto f2 = async([]() { return 2; }) && async([]() { return 3.5; });
    auto f2c = then(f2, [](cfuture<int> r1, cfuture<double> r2) { return r1.get() + static_cast<int>(r2.get()); });
    std::cout << f2c.get() << std::endl;

    // Unwrap values
    auto f3 = async([]() { return 2; }) && async([]() { return 3.5; });
    auto f3c = then(f3, [](int r1, double r2) { return r1 + static_cast<int>(r2); });
    std::cout << f3c.get() << std::endl;
