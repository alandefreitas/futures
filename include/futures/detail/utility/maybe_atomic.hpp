...
namespace futures {
    namespace detail {
        ...
        /**
         * Depending on the type of the operation state, in particular whether
         * its task is deferred, the synchronization primitives don't really
         * need atomic operations.
         *
         * This class encapsulates this logic so that real atomic operations can
         * be conditionally disabled. Only the operations necessary to atomic
        ...
        public:
            ...
            /// Atomically compares the object representation of *this with that
            /// of expected, and if those are bitwise-equal, replaces the former
            /// with desired
            /**
             * In the (2) and (4) versions order is used for both
             * read-modify-write and load operations, except that
             * std::memory_order_acquire and std::memory_order_relaxed are used
             * for the load operation if order == std::memory_order_acq_rel, or
             ...
             * @param order the memory synchronization ordering for both
             * operations
             *
             * @return true if the underlying atomic value was successfully
