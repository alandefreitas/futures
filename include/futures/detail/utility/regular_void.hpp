...
namespace futures {
    namespace detail {
        ...
        // Invoke the function f with the elements Is of a tuple where Is is a
        // list of mp_size_t. If the function returns void, this invoke function
        // returns a regular void.
        template <class F, class Tuple, class... Is>
        constexpr regular_tuple_invoke_result_t<F, Tuple>
        regular_tuple_invoke_impl(
            std::true_type, // is_regular_void<regular_tuple_invoke_result_t<F,
                            // Tuple>>
            F&& f,
            Tuple&& t,
            mp_list<Is...>) {
            ::futures::detail::invoke(
                std::forward<F>(f),
        ...
        constexpr regular_tuple_invoke_result_t<F, Tuple>
        regular_tuple_invoke_impl(
            std::false_type, // is_regular_void<regular_tuple_invoke_result_t<F,
                             // Tuple>>
            F&& f,
            Tuple&& t,
            mp_list<Is...>) {
            return ::futures::detail::invoke(
                std::forward<F>(f),
                std::get<Is::value>(std::forward<Tuple>(t))...);
        }
        ...
        regular_tuple_invoke(F&& f, Tuple&& t, mp_list<Is...>) {
            return regular_tuple_invoke_impl(
                is_regular_void<regular_tuple_invoke_result_t<F, Tuple>>{},
                std::forward<F>(f),
                std::forward<Tuple>(t),
                mp_list<Is...>{});
        }
        ...
        template <class F, class... Args>
        using regular_invoke_result_t =
            typename regular_invoke_result<F, Args...>::type;
        ...
        // result of make_irregular_tuple, which removes regular_void from the
        // list
        template <class... Args>
        ...
        template <class... Args>
        using make_irregular_tuple_result_t =
            typename make_irregular_tuple_result<Args...>::type;
        ...
        // returning regular void if return type is void
