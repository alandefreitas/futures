...
namespace futures {
    ...
    ///
    /// All promise types works as promise_base. The difference between the promise specializations
    /// is only in how they handle their set_value functions.
    ///
    /// \tparam R The shared state type
    template <typename R> class promise : public detail::promise_base<R> {
      public:
        ...
        void set_value(R const &value) {
            if (!detail::promise_base<R>::get_shared_state()) {
                throw promise_uninitialized{};
            }
            detail::promise_base<R>::get_shared_state()->set_value(value);
        }
        ...
        void set_value(R &&value) {
            ...
            detail::promise_base<R>::get_shared_state()->set_value(std::move(value));
        }

        /// \brief Swap the value of two promises
        void swap(promise &other) noexcept { detail::promise_base<R>::swap(other); }
    };
    ...
    template <> class promise<void> : public detail::promise_base<void> {
      public:
        ...
        void set_value() { // NOLINT(readability-make-member-function-const)
            if (!detail::promise_base<void>::get_shared_state()) {
                throw promise_uninitialized{};
            }
            detail::promise_base<void>::get_shared_state()->set_value();
        }

        /// \brief Swap the value of two promises
        void swap(promise &other) noexcept { detail::promise_base<void>::swap(other); }
    };
